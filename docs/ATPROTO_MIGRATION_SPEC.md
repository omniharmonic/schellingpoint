...
| **Derived reputation; attestations not scores** | `event_members.role` is deleted. `deriveRole(evidence, thresholds)` computes Visitor/Participant/Host/Organiser/Steward from evidence: has a profile, holds a ticket or invite, proposed a session that was scheduled, hosted a session with attendance, appointed. **No session is ever ranked in public by score.** The public tally carries counts and presence — `voterCount`, `totalVotes` — never an average, never a rank, never a leaderboard. The admin's sort-by-votes view is a members-only, organiser-only view. |
