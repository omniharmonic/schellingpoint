...
import { Loader2, Calendar, Heart, Clock, MapPin, Radio } from 'lucide-react'
...
/**
 * "Happening now" (design §11): while the attendance round is open, the sessions inside their
 * slot ± 15 min on the server's clock, each with the tap-to-vote control for the attendance
 * round. Saved sessions come first; the rest of the timed program follows so a session the
 * viewer walked into unplanned can still be voted for. Nothing here is a count.
 */
function HappeningNow({ eventSlug, timeZone, saved }: { eventSlug: string; timeZone: string; saved: Saved[] }) {
  ...
  return (
    <section aria-labelledby="happening-now-title" className="rounded-xl border border-primary/30 bg-primary/5 p-4">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-muted-foreground">Nothing is in session right now. Votes open 15 minutes before a session starts.</p>
      ) : (
                ...
                <CardContent className="flex items-start justify-between gap-4 p-4 pt-4">
                  <Link href={`/e/${eventSlug}/sessions/${session.id}`} className="min-w-0 flex-1 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                    <div className="mb-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      ...
                      {session.when ? (
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" aria-hidden />{formatTime(session.when.start_time, timeZone)}{session.when.end_time ? ` ${EN_DASH} ${formatTime(session.when.end_time, timeZone)}` : ''}</span>
                      ) : null}
                      {session.venue || session.is_self_hosted ? (
                        <span className="flex items-center gap-1"><MapPin className="h-3 w-3" aria-hidden />{session.is_self_hosted ? 'Self-hosted' : session.venue?.name}</span>
                      ) : null}
                    </div>
                    <h3 className="font-medium">{session.title}</h3>
                    <p className="text-sm text-muted-foreground">{hostByline(session)}</p>
                  </Link>
                  <VoteControl eventSlug={eventSlug} sessionId={session.id} sessionTitle={session.title} round="attendance" compact />
                </CardContent>
              </Card>
            </li>
          ))}
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <HappeningNow eventSlug={event.slug} timeZone={event.timezone} saved={favorites} />
