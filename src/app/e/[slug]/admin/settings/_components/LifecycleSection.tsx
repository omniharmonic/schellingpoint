...
export function LifecycleSection({ event, status, onChanged, hasIdentity }: LifecycleSectionProps) {
  ...
  const move = async (to: EventStatus) => {
    ...
    const result = await save({ status: to }, saved => to === 'voting_open'
      ...
      : status === 'draft'
        ? `Now ${STATUS_INFO[to].label.toLowerCase()}. Its public records are on the network.`
        ...
        return <li key={next} className={cn('rounded-xl border p-4', confirming && 'border-primary bg-primary/5')}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">{STATUS_INFO[next].description}.{next === 'voting_open' ? ' Every member is notified that voting is open.' : ''}{publishing ? ` This writes the gathering’s name, dates, description and rules as public records on the network${hasIdentity ? '' : ' (its network identity is created first)'}. Public records can be updated but not recalled.` : ''}{isIrreversibleTransition(next) ? ' This cannot be undone from here.' : ''}</p>
            </div>
