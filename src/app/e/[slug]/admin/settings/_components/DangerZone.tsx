...
export function DangerZone({ event, status }: { event: Event; status: EventStatus }) {
  ...
  return <SectionCard id="danger" title="Danger zone" description="Only the owner sees this. Deleting removes the gathering, its sessions, votes, members, and tickets for good." className="border-destructive/40">
    {deletable ? <>
    ...
    </> : <p className="text-sm text-muted-foreground">Only drafts can be deleted. This gathering has been published, so archive it from the Lifecycle section instead; archived gatherings leave discovery but keep their history.</p>}
  </SectionCard>
}
