...
export async function PATCH(request: Request, { params }: { params: { eventId: string } }) {
  ...
  const {data:member,error:memberError} = await db.from('event_members').select('role').eq('event_id',params.eventId).eq('user_id',user.id).maybeSingle()
  ...
  const {data:event,error} = await db.from('events').select('status').eq('id',params.eventId).single()
  ...
  }).eq('id',params.eventId).eq('status',event.status).select('id,status').maybeSingle()
