...
/** An explorable explanation, not a visualization of live participant data. */
export function GatheringArtwork({ compact = false }: { compact?: boolean }) {
  const [phase, setPhase] = useState(0)
  const current = phases[phase]
  return (
    <div className={cn('gathering-art relative rounded-[2rem] bg-[#eaf0eb] overflow-hidden', compact ? 'p-5' : 'p-6 sm:p-8')}>
      <svg viewBox="0 0 500 365" className="w-full" aria-hidden="true">
        ...
        <circle cx="250" cy="180" r={phase === 2 ? 92 : 65} fill="#dcd5ed" style={{ transition: 'r 350ms ease' }} />
        <g fill="#eaf0eb" stroke="currentColor" strokeWidth="1.4">
        ...
        <g fill="#202d2a" textAnchor="middle" fontFamily="BDO Grotesk, sans-serif">
        ...
        <g fontFamily="BDO Grotesk, sans-serif" fontSize="12" fill="#246653">
          <rect x="13" y="67" width="130" height="32" rx="16" fill="#fff" /><text x="78" y="87" textAnchor="middle">{current.labels[0]}</text>
          <rect x="340" y="149" width="146" height="32" rx="16" fill="#fff" /><text x="413" y="169" textAnchor="middle">{current.labels[1]}</text>
          <rect x="28" y="279" width="140" height="32" rx="16" fill="#fff" /><text x="98" y="299" textAnchor="middle">{current.labels[2]}</text>
        </g>
      </svg>
      {!compact && <>
        <div className="flex gap-2 mb-5" role="group" aria-label="How a gathering takes shape">
          {phases.map((item, index) => <button key={item.name} onClick={() => setPhase(index)} aria-pressed={phase === index} className="flex-1 rounded-full py-2.5 text-sm border border-[#246653]/20 transition-colors hover:bg-white/60">{item.name}</button>)}
        </div>
        <div aria-live="polite" className="min-h-[100px] text-[#202d2a]">
          <h2 className="text-lg font-semibold mb-1">{current.title}</h2>
          <p className="text-sm leading-relaxed text-[#50635a] max-w-sm">{current.description}</p>
        </div>
      </>}
    </div>
  )
